...
export default async function getSpeakers(): Promise<Speaker[]> {
  const res = await drupal.getResourceCollection<Speaker[]>(type, {
    params: params().getQueryObject(),
  });

  const res2 = await drupal.getResourceCollection<Speaker[]>(type, {
